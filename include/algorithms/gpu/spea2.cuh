#ifndef PMOO_GPU_SPEA2_CUH
#define PMOO_GPU_SPEA2_CUH
...
struct spea2_gpu {
    ...
    std::mt19937 random;
    // cuda
    struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM> *cuda_fitness_score;
...
void spea2_gpu_initialize(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2) {
    // initialize random number generators
    spea2->random.seed(std::random_device{}());

    // allocate memory on gpu
    CUDA_CALL(cudaMalloc(&spea2->cuda_fitness_score, sizeof(struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM>)));
    ...
    for (unsigned dim = 0u; dim < spea2->d_dim - spea2->i_dim; ++dim) {
        ...
        for (unsigned i = 0u; i < spea2->pop_size; ++i) {
            spea2->decision_variables[i][dim] = uniform(spea2->random);
        }
    }
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
double spea2_gpu_distance(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned individual1, const unsigned individual2) {
    double distance = 0.0;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void spea2_gpu_pareto_dominance(struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM> *spea2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void spea2_gpu_strength_score(struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM> *spea2) {
    const unsigned global_id_x = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned global_id_y = blockIdx.y * blockDim.y + threadIdx.y;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void spea2_gpu_calculate_distances(struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM> *spea2) {
    const unsigned global_id_x = blockIdx.x * blockDim.x + threadIdx.x;
    const unsigned global_id_y = blockIdx.y * blockDim.y + threadIdx.y;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void spea2_gpu_bitonic_sort_distances(struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM> *spea2, int j, int k) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void spea2_gpu_density_score(struct spea2_gpu_fs<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned k) {
...
void spea2_gpu_fitness_score(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned N) {
    ...
    spea2_gpu_pareto_dominance<<<spea2->BLOCKS_IN_GRID_2D, spea2->THREADS_IN_BLOCK_2D>>>(spea2->cuda_fitness_score);
    spea2_gpu_strength_score<<<spea2->BLOCKS_IN_GRID_2D, spea2->THREADS_IN_BLOCK_2D>>>(spea2->cuda_fitness_score);
    spea2_gpu_calculate_distances<<<spea2->BLOCKS_IN_GRID_2D, spea2->THREADS_IN_BLOCK_2D>>>(spea2->cuda_fitness_score);
    for (unsigned int k = 2; k <= N; k <<= 1)
        for (unsigned int j = k >> 1; j > 0; j >>= 1)
            spea2_gpu_bitonic_sort_distances<<<spea2->BLOCKS_IN_GRID_2D, spea2->THREADS_IN_BLOCK_2D>>>(spea2->cuda_fitness_score, j, k);
    spea2_gpu_density_score<<<spea2->BLOCKS_IN_GRID_1D, spea2->THREADS_IN_BLOCK_1D>>>(spea2->cuda_fitness_score, k);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_gpu_shuffle_selection(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2) {
    std::uniform_int_distribution<unsigned> dist(0u, spea2->pop_size - 1u);
    unsigned tmp, rnd;

    for (unsigned i = 0u; i < spea2->pop_size; ++i) {
        rnd = dist(spea2->random);
        tmp = spea2->shuffle[i];
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
unsigned spea2_gpu_tournament_selection(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned individual1, const unsigned individual2) {
    ...
    return ((uniform(spea2->random) < 0.5) ? individual1 : individual2);
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_gpu_simulated_binary_crossover(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned parent1_idx, const unsigned parent2_idx, const unsigned child1_idx, const unsigned child2_idx) {
    ...
    if (uniform(spea2->random) < spea2->p_crossover) { // crossover might not happen with crossover probability
        ...
        for (unsigned dim = 0u; dim < spea2->d_dim - spea2->i_dim; ++dim) { // for each decision variable in parents
            if (uniform(spea2->random) < 0.5 && fabs(spea2->decision_variables[parent1_idx][dim] - spea2->decision_variables[parent2_idx][dim]) > TOLERANCE_EPSILON) { // each variable has 50% chance to be crossovered
        ...
        if (spea2->i_dim > 0u) {
            ...
            unsigned cut1 = i_dim_dist(spea2->random);
            unsigned cut2 = i_dim_dist(spea2->random);
            if (cut1 < cut2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_gpu_polynomial_mutation(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned individual) {
    std::uniform_real_distribution<double> uniform(0.0, 1.0);

    // polynomial mutation - only for continous decision variables
    double deltad, delta1, delta2, rnd, mutation_exp, xy, val, deltaq;
    for (unsigned dim = 0u; dim < spea2->d_dim - spea2->i_dim; ++dim) {
        if (uniform(spea2->random) < spea2->p_mutation) { // mutation might not occur
    ...
    for (unsigned dim = spea2->d_dim - spea2->i_dim; dim < spea2->d_dim; ++dim) {
        ...
        spea2->decision_variables[individual][dim] = (double)dim_dist(spea2->random);
    }
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_gpu_environmental_selection(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2) {
    const unsigned N = 2u * spea2->pop_size;

    // calculate fitness score same as previously but with 2N population
    spea2_gpu_fitness_score(spea2, N);
    ...
    if (l <= spea2->pop_size) { // if there are exactly pop_size individuals with fitness score < 1 or less
        ...
        for (unsigned i = 0u; i < spea2->fitness_score_order.size; ++i) {
            for (unsigned j = i + 1u; j < spea2->fitness_score_order.size; ++j) {
                spea2->distance[i][j] = spea2_gpu_distance(spea2, spea2->fitness_score_order.elements[i], spea2->fitness_score_order.elements[j]);
        ...
        for (unsigned i = 0u; i < spea2->fitness_score_order.size; ++i) {
            for (unsigned j = 0u; j < spea2->fitness_score_order.size; ++j) spea2->distance_order[i][j] = j;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_gpu_evolve(struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2) {
    for (unsigned gen = 1u; gen <= spea2->gen; ++gen) {
        // prepare indexes for tournament selection
        spea2_gpu_shuffle_selection(spea2);

        // calculate fitness score
        spea2_gpu_fitness_score(spea2, spea2->pop_size);

        // selection, crossover and mutation (generate N children so that the population will reach 2N size)
        for (unsigned i = 0u; i < spea2->pop_size / 2u; ++i) {
            ...
            unsigned parent1_idx = spea2_gpu_tournament_selection(spea2, spea2->shuffle[selection_idx], spea2->shuffle[selection_idx + 1]);
            unsigned parent2_idx = spea2_gpu_tournament_selection(spea2, spea2->shuffle[selection_idx + 2], spea2->shuffle[selection_idx + 3]);
            // generate two new children by crossover of selected parents
            spea2_gpu_simulated_binary_crossover(spea2, parent1_idx, parent2_idx, child1_idx, child2_idx);
            // run mutation on both of new children
            spea2_gpu_polynomial_mutation(spea2, child1_idx);
            spea2_gpu_polynomial_mutation(spea2, child2_idx);
        ...
        spea2_gpu_environmental_selection(spea2);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_gpu_load_problem(void *self, Problem* problem) {
    struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_gpu_load_parameters(void *self, Parameters* parameters) {
    struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_gpu_initialize(void *self, void *gpu_self) {
    spea2_gpu_initialize((struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self);
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_gpu_evolve(void *self, void *gpu_self) {
    spea2_gpu_evolve((struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self);
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_gpu_cleanup(void *self, void *gpu_self) {
    struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_spea2_gpu_decision_variables(void *self) {
    struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_spea2_gpu_fitness_values(void *self) {
    struct spea2_gpu<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
Algorithm* pmoo_spea2_gpu_generate() {
    ...
    algorithm->decision_variables = pmoo_spea2_gpu_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_spea2_gpu_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_spea2_gpu_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_spea2_gpu_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_spea2_gpu_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_spea2_gpu_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_spea2_gpu_cleanup<POP_SIZE, D_DIM, F_DIM>;
