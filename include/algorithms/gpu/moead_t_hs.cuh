#ifndef PMOO_GPU_MOEAD_T_HS_CUH
#define PMOO_GPU_MOEAD_T_HS_CUH
...
__global__ void init_curand_hs_gpu_states(
    curandState* states,
...
__device__ double halton_gpu(unsigned index, unsigned base) {
    double f = 1.0;
    double r = 0.0;
    while (index > 0) {
        f /= base;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void generate_weight_vectors_halton_gpu_kernel(
    double* __restrict__ device_weight_vectors)
{
    ...
    for (unsigned d = 0; d < F_DIM - 1; ++d) {
        h[d] = halton_gpu(tid + 1, HALTON_PRIMES[d]);  // Halton uses 1-based index
    }
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void build_neighborhood_hs_gpu_kernel(
...
void moead_t_hs_gpu_initialize(struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead) {
    ...
    init_curand_hs_gpu_states<POP_SIZE,D_DIM,F_DIM><<<blocks, threads>>>(
        moead->deviceStates,
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_hs_gpu_evolve(struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead) {
    // 1D launch parameters
    auto tpb = moead->THREADS.x;
    auto bl  = moead->BLOCKS.x;

    generate_weight_vectors_halton_gpu_kernel<POP_SIZE, D_DIM, F_DIM><<<bl, tpb>>>(
        moead->device_weight_vectors);

    CUDA_CALL(cudaDeviceSynchronize());

    build_neighborhood_hs_gpu_kernel<POP_SIZE,D_DIM,F_DIM><<<bl,tpb>>>(
    ...
    for (unsigned gen = 0; gen < moead->gen; ++gen) {
        ...
        for (unsigned i = 0; i < moead->pop_size; ++i) {
           ...
           // Perform crossover and mutation to produce an offspring solution
            moead_t_hs_gpu_simulated_binary_crossover(moead, parent1, parent2, offspring_vars);
            moead_t_hs_gpu_mutation(moead, offspring_vars);
            ...
            for (unsigned k = 0; k < neighbor_size; ++k) {
                ...
                double f_current = moead_t_hs_gpu_calc_tchebycheff(moead, j, moead->fitness_values[j]);
                double f_offspring = moead_t_hs_gpu_calc_tchebycheff(moead, j, offspring_fitness);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
double moead_t_hs_gpu_calc_tchebycheff(struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead, unsigned subproblem_index, const double *obj_values) {
    double max_val = -1e30;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_hs_gpu_simulated_binary_crossover(struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead,
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
static void moead_t_hs_gpu_mutation(struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead, double *individual) {
    std::uniform_real_distribution<double> uniform(0.0, 1.0);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_hs_gpu_initialize(void *self, void *gpu_self) {
    moead_t_hs_gpu_initialize((struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_hs_gpu_load_parameters(void *self, Parameters* parameters) {
    struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
    ...
    &min_grid,
    &opt_bs,
    (void*)build_neighborhood_hs_gpu_kernel<POP_SIZE,D_DIM,F_DIM>,
    /*dynamic_smem=*/0,
    threads_per_block
    );
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_hs_gpu_load_problem(void *self, Problem* problem) {
    struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_hs_gpu_evolve(void *self, void *gpu_self) {
    moead_t_hs_gpu_evolve((struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_hs_gpu_cleanup(void *self, void *gpu_self) {
    struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_hs_gpu_decision_variables(void *self) {
    struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_hs_gpu_fitness_values(void *self) {
    struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>*)self;
    double **population = (double**)malloc(POP_SIZE * sizeof(double*));
    for (unsigned i = 0u; i < POP_SIZE; ++i)
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_moead_t_hs_gpu_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = MOEAD_T_HS;
    algorithm->variant = GPU;
    algorithm->size = sizeof(struct moead_t_hs_gpu<POP_SIZE, D_DIM, F_DIM>);
    algorithm->self = malloc(algorithm->size);
    algorithm->gpu_self = NULL;
    algorithm->population_size = POP_SIZE;
    algorithm->decision_variables = pmoo_moead_t_hs_gpu_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_moead_t_hs_gpu_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_moead_t_hs_gpu_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_moead_t_hs_gpu_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_moead_t_hs_gpu_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_moead_t_hs_gpu_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_moead_t_hs_gpu_cleanup<POP_SIZE, D_DIM, F_DIM>;
    return algorithm;
}
