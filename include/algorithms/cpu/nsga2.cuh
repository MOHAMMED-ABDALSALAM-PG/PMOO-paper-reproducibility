#ifndef PMOO_CPU_NSGA2_CUH
#define PMOO_CPU_NSGA2_CUH
...
void nsga2_cpu_fast_non_dominated_sorting(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned N) {
    ...
    for (OMP_FOR_INDEX i = 0u; i < N; ++i) {
        for (OMP_FOR_INDEX j = 0u; j < N; ++j) {
            if (pareto_dominance(nsga2->fitness_values[i], nsga2->fitness_values[j], nsga2->f_dim)) {
                ...
                omp_set_lock(&nsga2->lock[j]);
                nsga2->domination_count[j] += 1u;
                omp_unset_lock(&nsga2->lock[j]);
            }
        }
    }
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_cpu_crowding_distance(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2, unsigned front_idx) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_cpu_shuffle_selection(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
unsigned nsga2_cpu_tournament_selection(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned individual1, const unsigned individual2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_cpu_simulated_binary_crossover(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned parent1_idx, const unsigned parent2_idx, const unsigned child1_idx, const unsigned child2_idx) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_cpu_polynomial_mutation(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned individual) {
    std::uniform_real_distribution<double> uniform(0.0, 1.0);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_cpu_select_best_individuals(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2) {
    // run fnds and move individuals front-wise to the best vector only if an entire front fits in it
    nsga2_cpu_fast_non_dominated_sorting(nsga2, 2u * nsga2->pop_size);
    ...
    nsga2_cpu_crowding_distance(nsga2, last_front_idx);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_cpu_evolve(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2) {
    for (unsigned gen = 1u; gen <= nsga2->gen; ++gen) {
        // prepare indexes for tournament selection
        nsga2_cpu_shuffle_selection(nsga2);

        // calculate non dominated fronts and ranks
        nsga2_cpu_fast_non_dominated_sorting(nsga2, nsga2->pop_size);
        ...
        for (unsigned current_front_idx = 0u; current_front_idx < nsga2->pop_size; ++current_front_idx) {
            if (nsga2->non_dominated_front[current_front_idx].size == 0u) {
                ...
                nsga2_cpu_crowding_distance(nsga2, current_front_idx);
            }
        }
        ...
        for (OMP_FOR_INDEX i = 0u; i < nsga2->pop_size / 2u; ++i) {
            ...
            unsigned parent1_idx = nsga2_cpu_tournament_selection(nsga2, nsga2->shuffle[selection_idx], nsga2->shuffle[selection_idx + 1]);
            unsigned parent2_idx = nsga2_cpu_tournament_selection(nsga2, nsga2->shuffle[selection_idx + 2], nsga2->shuffle[selection_idx + 3]);
            // generate two new children by crossover of selected parents
            nsga2_cpu_simulated_binary_crossover(nsga2, parent1_idx, parent2_idx, child1_idx, child2_idx);
            // run mutation on both of new children
            nsga2_cpu_polynomial_mutation(nsga2, child1_idx);
            nsga2_cpu_polynomial_mutation(nsga2, child2_idx);
        ...
        nsga2_cpu_select_best_individuals(nsga2);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_cpu_load_problem(void *self, Problem* problem) {
    struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_cpu_load_parameters(void *self, Parameters* parameters) {
    struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_cpu_initialize(void *self, void *gpu_self) {
    nsga2_cpu_initialize((struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_cpu_evolve(void *self, void *gpu_self) {
    nsga2_cpu_evolve((struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_cpu_cleanup(void *self, void *gpu_self) {
    struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self;

    for (unsigned i = 0; i < POP_SIZE; ++i) omp_destroy_lock(&nsga2->lock[i]);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_nsga2_cpu_decision_variables(void *self) {
    struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_nsga2_cpu_fitness_values(void *self) {
    struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_nsga2_cpu_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = NSGA2;
    algorithm->variant = CPU;
    algorithm->size = sizeof(struct nsga2_cpu<POP_SIZE, D_DIM, F_DIM>);
    algorithm->self = malloc(algorithm->size);
    algorithm->gpu_self = NULL;
    algorithm->population_size = POP_SIZE;
    algorithm->decision_variables = pmoo_nsga2_cpu_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_nsga2_cpu_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_nsga2_cpu_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_nsga2_cpu_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_nsga2_cpu_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_nsga2_cpu_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_nsga2_cpu_cleanup<POP_SIZE, D_DIM, F_DIM>;
