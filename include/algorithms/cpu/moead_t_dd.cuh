#ifndef PMOO_CPU_MOEAD_T_DD_CUH
#define PMOO_CPU_MOEAD_T_DD_CUH
...
void moead_t_dd_cpu_initialize(struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_dd_cpu_evolve(struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead) {
    ...
    for (unsigned gen = 0; gen < moead->gen; ++gen) {
        ...
        for (OMP_FOR_INDEX i = 0; i < moead->pop_size; ++i) {
           ...
           // Perform crossover and mutation to produce an offspring solution
            moead_t_dd_cpu_simulated_binary_crossover(moead, parent1, parent2, offspring_vars, i);
            moead_t_dd_cpu_mutation(moead, offspring_vars, i);
            ...
            for (unsigned k = 0; k < neighbor_size; ++k) {
                ...
                double f_current = moead_t_dd_cpu_calc_tchebycheff(moead, j, moead->fitness_values[j]);
                double f_offspring = moead_t_dd_cpu_calc_tchebycheff(moead, j, offspring_fitness);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
double moead_t_dd_cpu_calc_tchebycheff(struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead, unsigned subproblem_index, const double *obj_values) {
    double max_val = -1e30;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_dd_cpu_simulated_binary_crossover(struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead,
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
static void moead_t_dd_cpu_mutation(struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead, double *individual, unsigned index) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_cpu_initialize(void *self, void *gpu_self) {
    moead_t_dd_cpu_initialize((struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_cpu_load_parameters(void *self, Parameters* parameters) {
    struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_cpu_load_problem(void *self, Problem* problem) {
    struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_cpu_evolve(void *self, void *gpu_self) {
    moead_t_dd_cpu_evolve((struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_cpu_cleanup(void *self, void *gpu_self) {
    struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
    for (unsigned i = 0; i < POP_SIZE; ++i) omp_destroy_lock(&moead->lock[i]);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_dd_cpu_decision_variables(void *self) {
    struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_dd_cpu_fitness_values(void *self) {
    struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>*)self;
    double **population = (double**)malloc(POP_SIZE * sizeof(double*));
    for (unsigned i = 0; i < POP_SIZE; ++i) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_moead_t_dd_cpu_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = MOEAD_T_DD;
    algorithm->variant = CPU;
    algorithm->size = sizeof(struct moead_t_dd_cpu<POP_SIZE, D_DIM, F_DIM>);
    algorithm->self = malloc(algorithm->size);
    algorithm->gpu_self = NULL;
    algorithm->population_size = POP_SIZE;
    algorithm->decision_variables = pmoo_moead_t_dd_cpu_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_moead_t_dd_cpu_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_moead_t_dd_cpu_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_moead_t_dd_cpu_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_moead_t_dd_cpu_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_moead_t_dd_cpu_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_moead_t_dd_cpu_cleanup<POP_SIZE, D_DIM, F_DIM>;
    return algorithm;
}
