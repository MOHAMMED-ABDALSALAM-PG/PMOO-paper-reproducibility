#ifndef PMOO_SERIAL_SPEA2_CUH
#define PMOO_SERIAL_SPEA2_CUH
...
void spea2_serial_initialize(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
double spea2_serial_distance(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned individual1, const unsigned individual2) {
    double distance = 0.0;
...
void spea2_serial_fitness_score(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned N) {
    ...
    for (unsigned i = 0u; i < N; ++i) {
        for (unsigned j = 0u; j < i; ++j) {
            ...
            } else if (pareto_dominance(spea2->fitness_values[j], spea2->fitness_values[i], spea2->f_dim)) {
                spea2->domination_list[i].elements[spea2->domination_list[i].size] = j;
                spea2->domination_list[i].size += 1u;
                spea2->domination_count[j] += 1u;
            }
        }
    }
    ...
    for (unsigned i = 0u; i < N; ++i) {
        for (unsigned j = i + 1u; j < N; ++j) {
            spea2->distance[i][j] = spea2_serial_distance(spea2, i, j);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_serial_shuffle_selection(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
unsigned spea2_serial_tournament_selection(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned individual1, const unsigned individual2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_serial_simulated_binary_crossover(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned parent1_idx, const unsigned parent2_idx, const unsigned child1_idx, const unsigned child2_idx) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_serial_polynomial_mutation(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2, const unsigned individual) {
    std::uniform_real_distribution<double> uniform(0.0, 1.0);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_serial_environmental_selection(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2) {
    const unsigned N = 2u * spea2->pop_size;

    // calculate fitness score same as previously but with 2N population
    spea2_serial_fitness_score(spea2, N);
    ...
    if (l <= spea2->pop_size) { // if there are exactly pop_size individuals with fitness score < 1 or less
        ...
        for (unsigned i = 0u; i < spea2->fitness_score_order.size; ++i) {
            for (unsigned j = i + 1u; j < spea2->fitness_score_order.size; ++j) {
                spea2->distance[i][j] = spea2_serial_distance(spea2, spea2->fitness_score_order.elements[i], spea2->fitness_score_order.elements[j]);
                spea2->distance[j][i] = spea2->distance[i][j]; // the matrix is symmetric
            }
        }
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void spea2_serial_evolve(struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2) {
    for (unsigned gen = 1u; gen <= spea2->gen; ++gen) {
        // prepare indexes for tournament selection
        spea2_serial_shuffle_selection(spea2);

        // calculate fitness score
        spea2_serial_fitness_score(spea2, spea2->pop_size);
        ...
        for (unsigned i = 0u; i < spea2->pop_size / 2u; ++i) {
            ...
            unsigned parent1_idx = spea2_serial_tournament_selection(spea2, spea2->shuffle[selection_idx], spea2->shuffle[selection_idx + 1]);
            unsigned parent2_idx = spea2_serial_tournament_selection(spea2, spea2->shuffle[selection_idx + 2], spea2->shuffle[selection_idx + 3]);
            // generate two new children by crossover of selected parents
            spea2_serial_simulated_binary_crossover(spea2, parent1_idx, parent2_idx, child1_idx, child2_idx);
            // run mutation on both of new children
            spea2_serial_polynomial_mutation(spea2, child1_idx);
            spea2_serial_polynomial_mutation(spea2, child2_idx);
        ...
        spea2_serial_environmental_selection(spea2);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_serial_load_problem(void *self, Problem* problem) {
    struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_serial_load_parameters(void *self, Parameters* parameters) {
    struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_serial_initialize(void *self, void *gpu_self) {
    spea2_serial_initialize((struct spea2_serial<POP_SIZE, D_DIM, F_DIM>*)self);
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_serial_evolve(void *self, void *gpu_self) {
    spea2_serial_evolve((struct spea2_serial<POP_SIZE, D_DIM, F_DIM>*)self);
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_spea2_serial_cleanup(void *self, void *gpu_self) {
    return;
};

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_spea2_serial_decision_variables(void *self) {
    struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_spea2_serial_fitness_values(void *self) {
    struct spea2_serial<POP_SIZE, D_DIM, F_DIM> *spea2 = (struct spea2_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_spea2_serial_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = SPEA2;
    algorithm->variant = SERIAL;
    algorithm->size = sizeof(struct spea2_serial<POP_SIZE, D_DIM, F_DIM>);
    algorithm->self = malloc(algorithm->size);
    algorithm->gpu_self = NULL;
    algorithm->population_size = POP_SIZE;
    algorithm->decision_variables = pmoo_spea2_serial_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_spea2_serial_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_spea2_serial_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_spea2_serial_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_spea2_serial_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_spea2_serial_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_spea2_serial_cleanup<POP_SIZE, D_DIM, F_DIM>;
