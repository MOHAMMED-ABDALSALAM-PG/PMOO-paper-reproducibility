#ifndef PMOO_SERIAL_MOEAD_T_DD_CUH
#define PMOO_SERIAL_MOEAD_T_DD_CUH
...
struct moead_t_dd_serial {
    ...
    // MOEA/D-specific                                     // Size of neighborhood for each subproblem (MOEAD_NEIGHBORHOOD_SIZE)
...
void moead_t_dd_serial_initialize(struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_dd_serial_evolve(struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead) {
    // Determine H for weight vector grid: smallest H such that C(H+F-1, F-1) >= POP_SIZE
    unsigned k = (moead->f_dim > 0 ? moead->f_dim - 1 : 0);
    ...
    do {
        H++;
    } while (comb(H + moead->f_dim - 1, k) < moead->pop_size);

    unsigned count = 0;

    auto generate_weights = [&](auto &&self, int idx, int remaining, int local_current[]) -> void {
        if (idx == (int)moead->f_dim - 1) {
            local_current[idx] = remaining;

            // Safely write to shared weight_vectors
            if (count >= moead->pop_size) {
                return;
            }
            unsigned index = count++;

            for (unsigned j = 0; j < moead->f_dim; ++j) {
                moead->weight_vectors[index][j] = static_cast<double>(local_current[j]) / H;
            }
            return;
        }
        ...
        for (int i = 0; i <= remaining; ++i) {
            local_current[idx] = i;
            self(self, idx + 1, remaining - i, local_current);
        }
    };

    for (int i = 0; i <= H; ++i) {
        int local_current[POP_SIZE];
        local_current[0] = i;
        generate_weights(generate_weights, 1, H - i, local_current);
    }
    ...
    for (unsigned gen = 0; gen < moead->gen; ++gen) {
        ...
        for (unsigned i = 0; i < moead->pop_size; ++i) {
           ...
           // Perform crossover and mutation to produce an offspring solution
            moead_t_dd_serial_simulated_binary_crossover(moead, parent1, parent2, offspring_vars);
            moead_t_dd_mutation(moead, offspring_vars);
            // Evaluate the offspring's objectives
            moead->fitness(offspring_vars, offspring_fitness, moead->d_dim, moead->f_dim);
            //moead->problem->evaluate(offspring_vars, offspring_fitness);
            ...
            for (unsigned k = 0; k < neighbor_size; ++k) {
                ...
                double f_current = moead_t_dd_calc_tchebycheff(moead, j, moead->fitness_values[j]);
                double f_offspring = moead_t_dd_calc_tchebycheff(moead, j, offspring_fitness);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
double moead_t_dd_calc_tchebycheff(struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead, unsigned subproblem_index, const double *obj_values) {
    double max_val = -1e30;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_dd_serial_simulated_binary_crossover(struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead,
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
static void moead_t_dd_mutation(struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead, double *individual) {
    unsigned D = moead->d_dim;
    double pm = moead->p_mutation;
    double eta_m = moead->eta_mutation;
    std::uniform_real_distribution<double> uniform(0.0, 1.0);
    std::mt19937 &rng = moead->randState;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_serial_initialize(void *self, void *gpu_self) {
    moead_t_dd_serial_initialize((struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_serial_load_parameters(void *self, Parameters* parameters) {
    struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_serial_load_problem(void *self, Problem* problem) {
    struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_serial_evolve(void *self, void *gpu_self) {
    moead_t_dd_serial_evolve((struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_serial_cleanup(void *self, void *gpu_self) {
    return;
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_dd_serial_decision_variables(void *self) {
    struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_dd_serial_fitness_values(void *self) {
    struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>*)self;
    double **population = (double**)malloc(POP_SIZE * sizeof(double*));
    for (unsigned i = 0; i < POP_SIZE; ++i) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_moead_t_dd_serial_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = MOEAD_T_DD;
    algorithm->variant = SERIAL;
    algorithm->size = sizeof(struct moead_t_dd_serial<POP_SIZE, D_DIM, F_DIM>);
    algorithm->self = malloc(algorithm->size);
    algorithm->gpu_self = NULL;
    algorithm->population_size = POP_SIZE;
    algorithm->decision_variables = pmoo_moead_t_dd_serial_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_moead_t_dd_serial_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_moead_t_dd_serial_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_moead_t_dd_serial_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_moead_t_dd_serial_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_moead_t_dd_serial_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_moead_t_dd_serial_cleanup<POP_SIZE, D_DIM, F_DIM>;
    return algorithm;
}
