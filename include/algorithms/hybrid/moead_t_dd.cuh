#ifndef PMOO_HYBRID_MOEAD_T_DD_CUH
#define PMOO_HYBRID_MOEAD_T_DD_CUH
...
__global__ void init_curand_dd_hybrid_states(
    curandState* states,
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void generate_weight_vectors_grid_kernel(
    double* __restrict__ device_weight_vectors,
    const unsigned H,
    const unsigned total_combinations) {

    unsigned tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= POP_SIZE || tid >= total_combinations) return;

    unsigned n = tid;
    unsigned sum = H;
    unsigned result[F_DIM] = {0};

    for (unsigned i = 0; i < F_DIM - 1; ++i) {
        unsigned x = 0;
        while (true) {
            // Remaining compositions of (sum - x) into (F_DIM - i - 1)
            unsigned r = F_DIM - i - 2;
            unsigned c = 1;
    ...
    // Normalize to [0,1] simplex
    for (unsigned j = 0; j < F_DIM; ++j)
        device_weight_vectors[tid * F_DIM + j] = static_cast<double>(result[j]) / H;
}


template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void build_neighborhood_dd_hybrid_kernel(
...
void moead_t_dd_hybrid_initialize(struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_dd_hybrid_evolve(struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead) {
    auto tpb = moead->THREADS.x;
    auto bl  = moead->BLOCKS.x;

    // Host-side combinatorics to determine H
    unsigned k = (F_DIM > 0 ? F_DIM - 1 : 0);
    auto comb = [](unsigned n, unsigned r) {
        double c = 1.0;
        for (unsigned i = 1; i <= r; ++i)
            c = c * (n - r + i) / i;
        return static_cast<unsigned>(c + 0.5);
    };

    unsigned H = 0;
    while (comb(H + F_DIM - 1, k) < POP_SIZE) H++;
    unsigned total_combinations = comb(H + F_DIM - 1, k);

    // Launch grid-based weight generation kernel
    generate_weight_vectors_grid_kernel<POP_SIZE, D_DIM, F_DIM><<<bl, tpb>>>(
        moead->device_weight_vectors, H, total_combinations);
    CUDA_CALL(cudaDeviceSynchronize());


    build_neighborhood_dd_hybrid_kernel<POP_SIZE,D_DIM,F_DIM><<<bl,tpb>>>(
    ...
    for (unsigned gen = 0; gen < moead->gen; ++gen) {
        ...
        for (OMP_FOR_INDEX i = 0; i < POP_SIZE; ++i) {
           ...
           // Perform crossover and mutation to produce an offspring solution
            moead_t_dd_hybrid_simulated_binary_crossover(moead, parent1, parent2, offspring_vars, i);
            moead_t_dd_hybrid_mutation(moead, offspring_vars, i);
            ...
            for (unsigned k = 0; k < neighbor_size; ++k) {
                ...
                double f_current = moead_t_dd_hybrid_calc_tchebycheff(moead, j, moead->fitness_values[j]);
                double f_offspring = moead_t_dd_hybrid_calc_tchebycheff(moead, j, offspring_fitness);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
double moead_t_dd_hybrid_calc_tchebycheff(struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead, unsigned subproblem_index, const double *obj_values) {
    double max_val = -1e30;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void moead_t_dd_hybrid_simulated_binary_crossover(struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead,
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
static void moead_t_dd_hybrid_mutation(struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead, double *individual, unsigned index) {
    std::uniform_real_distribution<double> uniform(0.0, 1.0);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_hybrid_initialize(void *self, void *gpu_self) {
    moead_t_dd_hybrid_initialize((struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_hybrid_load_parameters(void *self, Parameters* parameters) {
    struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_hybrid_load_problem(void *self, Problem* problem) {
    struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_hybrid_evolve(void *self, void *gpu_self) {
    moead_t_dd_hybrid_evolve((struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_moead_t_dd_hybrid_cleanup(void *self, void *gpu_self) {
    struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_dd_hybrid_decision_variables(void *self) {
    struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_moead_t_dd_hybrid_fitness_values(void *self) {
    struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM> *moead = (struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
    double **population = (double**)malloc(POP_SIZE * sizeof(double*));
    for (unsigned i = 0u; i < POP_SIZE; ++i)
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_moead_t_dd_hybrid_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = MOEAD_T_DD;
    algorithm->variant = HYBRID;
    algorithm->size = sizeof(struct moead_t_dd_hybrid<POP_SIZE, D_DIM, F_DIM>);
    algorithm->self = malloc(algorithm->size);
    algorithm->gpu_self = NULL;
    algorithm->population_size = POP_SIZE;
    algorithm->decision_variables = pmoo_moead_t_dd_hybrid_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_moead_t_dd_hybrid_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_moead_t_dd_hybrid_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_moead_t_dd_hybrid_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_moead_t_dd_hybrid_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_moead_t_dd_hybrid_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_moead_t_dd_hybrid_cleanup<POP_SIZE, D_DIM, F_DIM>;
    return algorithm;
}
