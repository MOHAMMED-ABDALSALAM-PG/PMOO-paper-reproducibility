#ifndef PMOO_HYBRID_NSGA2_CUH
#define PMOO_HYBRID_NSGA2_CUH
...
struct nsga2_hybrid {
    ...
    // cuda
    struct nsga2_hybrid_fnds<POP_SIZE, D_DIM, F_DIM> *cuda_fnds;                   // Store pointer to the gpu memory
...
void nsga2_hybrid_initialize(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2) {
    // initialize omp
    omp_set_num_threads(nsga2->num_threads);
        
    // initialize random number generators
    for (unsigned i = 0u; i < 2u * nsga2->pop_size; ++i)
        nsga2->random[i].seed(std::random_device{}());

    // allocate memory on gpu
    CUDA_CALL(cudaMalloc(&nsga2->cuda_fnds, sizeof(struct nsga2_hybrid_fnds<POP_SIZE, D_DIM, F_DIM>)));
    ...
    for (unsigned dim = 0u; dim < nsga2->d_dim - nsga2->i_dim; ++dim) {
        ...
        for (unsigned i = 0u; i < nsga2->pop_size; ++i) {
            nsga2->decision_variables[i][dim] = uniform(nsga2->random[i]);
        }
    }
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
__global__ void nsga2_hybrid_fnds_pareto_dominance(struct nsga2_hybrid_fnds<POP_SIZE, D_DIM, F_DIM> * nsga2) {
    ...
    if (pareto_dominance(nsga2->fitness_values[global_id_x], nsga2->fitness_values[global_id_y], F_DIM)) {
        unsigned idx = atomicAdd(&nsga2->domination_list[global_id_x].size, 1u);
        nsga2->domination_list[global_id_x].elements[idx] = global_id_y;
        atomicAdd(&nsga2->domination_count[global_id_y], 1u);
    }
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
// Includes parameter N to make it usable on population sizes N and 2N
void nsga2_hybrid_fast_non_dominated_sorting(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned N) {
    ...
    // calculate required amount of blocks to call kernel
    nsga2->BLOCKS_IN_GRID_2D.x = N / nsga2->THREADS_IN_BLOCK_2D.x;
    nsga2->BLOCKS_IN_GRID_2D.y = N / nsga2->THREADS_IN_BLOCK_2D.y;
    nsga2->BLOCKS_IN_GRID_2D.z = 1u;

    CUDA_CALL(cudaMemcpy(nsga2->cuda_fnds, nsga2->fitness_values, sizeof(struct nsga2_hybrid_fnds<POP_SIZE, D_DIM, F_DIM>), cudaMemcpyHostToDevice));
    nsga2_hybrid_fnds_pareto_dominance<<<nsga2->BLOCKS_IN_GRID_2D, nsga2->THREADS_IN_BLOCK_2D>>>(nsga2->cuda_fnds);
    CUDA_CALL(cudaMemcpy(nsga2->fitness_values, nsga2->cuda_fnds, sizeof(struct nsga2_hybrid_fnds<POP_SIZE, D_DIM, F_DIM>), cudaMemcpyDeviceToHost));
    ...
    for (unsigned i = 0u; i < N; ++i) {
        if (nsga2->domination_count[i] == 0u) { // if no individual dominates i-th individual
            ...
            // individuals from the first front have their ranks implicitly set to 0u
        }
    }
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_hybrid_crowding_distance(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2, unsigned front_idx) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_hybrid_shuffle_selection(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2) {
    std::uniform_int_distribution<unsigned> dist(0u, nsga2->pop_size - 1u);
    unsigned tmp, rnd;

    for (unsigned i = 0u; i < nsga2->pop_size; ++i) {
        rnd = dist(nsga2->random[i]);
        tmp = nsga2->shuffle[i];
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
unsigned nsga2_hybrid_tournament_selection(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned individual1, const unsigned individual2) {
    ...
    return ((uniform(nsga2->random[individual1]) < 0.5) ? individual1 : individual2);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_hybrid_simulated_binary_crossover(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned parent1_idx, const unsigned parent2_idx, const unsigned child1_idx, const unsigned child2_idx) {
std::uniform_real_distribution<double> uniform(0.0, 1.0);
    ...
    if (uniform(nsga2->random[parent1_idx]) < nsga2->p_crossover) { // crossover might not happen with crossover probability
        ...
        for (unsigned dim = 0u; dim < nsga2->d_dim - nsga2->i_dim; ++dim) { // for each decision variable in parents
            if (uniform(nsga2->random[parent1_idx]) < 0.5 && fabs(nsga2->decision_variables[parent1_idx][dim] - nsga2->decision_variables[parent2_idx][dim]) > TOLERANCE_EPSILON) { // each variable has 50% chance to be crossovered
                ...
                rnd = uniform(nsga2->random[parent1_idx]);
                ...
                if (uniform(nsga2->random[parent1_idx]) < 0.5) {
        ...
        if (nsga2->i_dim > 0u) {
            ...
            unsigned cut1 = i_dim_dist(nsga2->random[parent1_idx]);
            unsigned cut2 = i_dim_dist(nsga2->random[parent1_idx]);
            if (cut1 < cut2) {
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_hybrid_polynomial_mutation(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2, const unsigned individual) {
    std::uniform_real_distribution<double> uniform(0.0, 1.0);

    // polynomial mutation - only for continous decision variables
    double deltad, delta1, delta2, rnd, mutation_exp, xy, val, deltaq;
    for (unsigned dim = 0u; dim < nsga2->d_dim - nsga2->i_dim; ++dim) {
        if (uniform(nsga2->random[individual]) < nsga2->p_mutation) { // mutation might not occur
            ...
            rnd = uniform(nsga2->random[individual]);
            if (rnd < 0.5) {
                xy = 1.0 - delta1;
    ...
    for (unsigned dim = nsga2->d_dim - nsga2->i_dim; dim < nsga2->d_dim; ++dim) {
        ...
        nsga2->decision_variables[individual][dim] = (double)dim_dist(nsga2->random[individual]);
    }
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_hybrid_select_best_individuals(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2) {
    // run fnds and move individuals front-wise to the best vector only if an entire front fits in it
    nsga2_hybrid_fast_non_dominated_sorting(nsga2, 2u * nsga2->pop_size);
    ...
    nsga2_hybrid_crowding_distance(nsga2, last_front_idx);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
void nsga2_hybrid_evolve(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2) {
    for (unsigned gen = 1u; gen <= nsga2->gen; ++gen) {
        // prepare indexes for tournament selection
        nsga2_hybrid_shuffle_selection(nsga2);

        // calculate non dominated fronts and ranks
        nsga2_hybrid_fast_non_dominated_sorting(nsga2, nsga2->pop_size);
        ...
        for (unsigned current_front_idx = 0u; current_front_idx < nsga2->pop_size; ++current_front_idx) {
            if (nsga2->non_dominated_front[current_front_idx].size == 0u) {
                ...
                nsga2_hybrid_crowding_distance(nsga2, current_front_idx);
            }
        }

        // selection, crossover and mutation (generate N children so that the population will reach 2N size)
        #pragma omp parallel for
        for (OMP_FOR_INDEX i = 0u; i < nsga2->pop_size / 2u; ++i) {
            ...
            unsigned parent1_idx = nsga2_hybrid_tournament_selection(nsga2, nsga2->shuffle[selection_idx], nsga2->shuffle[selection_idx + 1]);
            unsigned parent2_idx = nsga2_hybrid_tournament_selection(nsga2, nsga2->shuffle[selection_idx + 2], nsga2->shuffle[selection_idx + 3]);
            // generate two new children by crossover of selected parents
            nsga2_hybrid_simulated_binary_crossover(nsga2, parent1_idx, parent2_idx, child1_idx, child2_idx);
            // run mutation on both of new children
            nsga2_hybrid_polynomial_mutation(nsga2, child1_idx);
            nsga2_hybrid_polynomial_mutation(nsga2, child2_idx);
        ...
        nsga2_hybrid_select_best_individuals(nsga2);
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_hybrid_load_problem(void *self, Problem* problem) {
    struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_hybrid_load_parameters(void *self, Parameters* parameters) {
    struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
    ...
    nsga2->THREADS_IN_BLOCK_2D.x = ceil(sqrt(parameters->gpu_block_size));
    nsga2->THREADS_IN_BLOCK_2D.y = ceil(sqrt(parameters->gpu_block_size));
    nsga2->THREADS_IN_BLOCK_2D.z = 1u;

    nsga2->num_threads = parameters->cpu_threads;
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_hybrid_initialize(void *self, void *gpu_self) {
    nsga2_hybrid_initialize((struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_hybrid_cleanup(void *self, void *gpu_self) {
    struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;

    cudaFree(nsga2->cuda_fnds);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline void pmoo_nsga2_hybrid_evolve(void *self, void *gpu_self) {
    nsga2_hybrid_evolve((struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self);
}

template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_nsga2_hybrid_decision_variables(void *self) {
    struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
inline double** pmoo_nsga2_hybrid_fitness_values(void *self) {
    struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM> *nsga2 = (struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>*)self;
...
template <unsigned POP_SIZE, unsigned D_DIM, unsigned F_DIM>
Algorithm* pmoo_nsga2_hybrid_generate() {
    Algorithm *algorithm = (Algorithm*)malloc(sizeof(Algorithm));

    algorithm->name = NSGA2;
    algorithm->variant = HYBRID;
    algorithm->size = sizeof(struct nsga2_hybrid<POP_SIZE, D_DIM, F_DIM>);
    ...
    algorithm->decision_variables = pmoo_nsga2_hybrid_decision_variables<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->fitness_values = pmoo_nsga2_hybrid_fitness_values<POP_SIZE, D_DIM, F_DIM>(algorithm->self);
    algorithm->load_problem = pmoo_nsga2_hybrid_load_problem<POP_SIZE, D_DIM, F_DIM>;
    algorithm->load_parameters = pmoo_nsga2_hybrid_load_parameters<POP_SIZE, D_DIM, F_DIM>;
    algorithm->initialize = pmoo_nsga2_hybrid_initialize<POP_SIZE, D_DIM, F_DIM>;
    algorithm->evolve = pmoo_nsga2_hybrid_evolve<POP_SIZE, D_DIM, F_DIM>;
    algorithm->cleanup = pmoo_nsga2_hybrid_cleanup<POP_SIZE, D_DIM, F_DIM>;
